...
has 'path' => ( isa => 'Maybe[Str]', is => 'ro', required => 1 )
    ; # path is the full path/url to locate the file on the medium, whichever it is source or repository
has 'rel_path' => ( isa => 'Str', is => 'ro', required => 1 )
    ;    # rel_path is the relative path in the source
...
sub BUILD {
    ...
    if ( ref($self) eq 'Rdiffopke::File' ) {
        Rdiffopke::Exception::File->throw( error =>
