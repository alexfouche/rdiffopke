...
has 'prev_rdiff' => ( is => 'ro', isa => 'Int', writer => '_set_rdiff' )
...
sub _set_current_rdiff {
    ...
    try {
        $rdiff =
          $self->_dbh->selectrow_array('select max(rdiff) from rdiffs;') );
    }
    catch {
          Rdiffopke::Exception::Metadata->throw( error =>
                "Could not read latest rdiff number from metadata file:\n"
                . $self->_dbh->errstr );
    };
      ( defined $rdiff )
      ? ( $self->_set_rdiff($rdiff) )
      : ( $self->_set_rdiff(0) );
}
...
sub add_rdiff {
      ...
      $self->prev_rdiff( $self->rdiff );

      # Add a new rdiff number in metadata
      try {
          $self->_dbh->begin_work;
          $self->_dbh->do(
                  "insert into rdiffs(rdiff, date_begin, message) values("
                . ( $self->prev_rdiff + 1 ) . ", "
                . gmtime(time)
                . ", 'Adding new rdiff' ;" );
      }
...
"update options set value = $i where name = 'metadata_version';"
              ...
              $self->_dbh->rollback;
              Rdiffopke::Exception::Metadata->throw( error =>
                    "An error occurred while upgrading metadata schema:\n"
                    . $self->_dbh->errstr );
          };
          $self->_dbh->commit;
...
"select file_id, paths.path, localfiles.path, owner, 'group', mode, mtime, files.size from files, paths, localfiles where rdiff=$rdiff and files.path_id=paths.path_id and files.localfile_id=localfiles.localfile_id;"
          );
      }
      catch {
          Rdiffopke::Exception::Metadata->throw(
...
sub _add_file_metadata {
      ...
      my $file    = shift;    # Should be a Rdiffopke::File
      my $path_id = shift
        , # An optionnal path_id to be given instead of creating a new record in 'paths' table
        my $localfile =
        shift # localfile is a small array [localpath, 'mtime', 'size'], needed if we want to insert a file transferred to repository
...
# We are given a 'localfiles' table record structure, let's create it and put the reference in the 'files' record
          my $localfile_id;
          unless ( defined $localfile ) {

              # First add a localfile entry in database
              $self->_dbh->do(
                      'insert into localfiles(path, mdate, size) values("'
                    . $localfile->[0] . '", "'
                    . $localfile->[1] . '",'
                    . $localfile->[2] . ');';
              ...
              if ($localfile_id) {
                  $localfile_id_string1 =
                    "localfile_id," $localfile_id_string2 =
                    "localfile_id=$localfile_id , ";
              }

              $self->_dbh->do(
"insert into files(rdiff, path_id, $localfile_id_string1 owner, 'group', mode, mtime, type, size) values("
                    . $self->rdiff . ","
                    . $path_id
