...
has 'schema_version' => ( is => 'ro', isa => 'Int', writer => '_set_schema_version' );
...
sub add_rdiff {
    ...
    try {
        ...
        $self->_dbh->do( "insert into rdiffs(rdiff, date_begin, date_end, message) values("
                . ( $self->prev_rdiff + 1 ) . ", '"
                . scalar( gmtime(time) )
                . "','', 'Adding new rdiff' );" );
    }
...
sub elevate_files_to_last_rdiff {
    ...
    try {
        $self->_dbh->begin_work;
        $self->_dbh->do(
            "update files set rdiff=" . $self->rdiff . " where rdiff=" . $self->prev_rdiff . ";" );
    }
    catch {
        $self->_dbh->rollback;
        Rdiffopke::Exception::Metadata->throw(
            error => "An error occurred while updating rdiffs for files in metadata\n"
                . $self->_dbh->errstr );
    };
    $self->_dbh->commit;
}
...
sub _connect {
    ...
    unless ( defined $dbh ) {
        Rdiffopke::Exception::Metadata->throw(
            error => "Metadata file seems to be corrupted. It should be a SQLite database\n" );
    }
...
sub _upgrade_schema {
    ...
    for ( my $i = $self->schema_version + 1 ; $i <= $self->upgrade_to ; $i++ ) {
        ...
        try {
            ...
            $self->_dbh->do("update options set value = $i where name = 'metadata_version';");
        }
        catch {
            $self->_dbh->rollback;
            Rdiffopke::Exception::Metadata->throw(
...
sub set_message {
    ...
    try {
        $self->_dbh->do( "update rdiffs set message = '$message' where rdiff = " . $self->rdiff )
    ...
    foreach ( @$sql_rows_files, @$sql_rows_others ) {
        $file_list->add(
            ...
            Rdiffopke::File::_LocalFile->new(
                file_id  => $_->[0],
                rel_path => $_->[1],    # represents the relative path on the source
...
sub _discard_file {
    ...
    if ( defined $file->file_id ) {
        ...
        $path_id =
            $self->_dbh->selectrow_array("select path_id from files where file_id=$file_id;");
    } else {
...
sub _add_file {
    ...
    my $file = shift;    # Should be a Rdiffopke::File
    my $path_id =
        shift;   # An optional path_id to be given instead of creating a new record in 'paths' table
    my $localfile = shift
...
# it would take less space in DB if we have a lot of long paths and a lot of file changes between runs of rdiffopke
        $path_id = $self->_dbh->selectrow_array(
            'select max(path_id) from paths where path="' . $file->rel_path . '" ;' );
        unless ($path_id) {
    ...
    if ( defined $localfile && $file->is_file ) {
        ...
        $localfile_id = $self->_dbh->selectrow_array(
            'select max(localfile_id) from localfiles where path="' . $localfile->[0] . '";' );
        unless ($localfile_id) {
            Rdiffopke::Exception::Metadata->throw(
...
sub discard_add_file {
    ...
    my $file      = shift;    # Should be a Rdiffopke::File
    my $localfile = shift
        ;    # localfile is a -optional- small array [localpath, 'mtime', 'size', optional path_id]
